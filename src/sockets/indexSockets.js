...
const setUpSockets = (app) => {
  ...
  gameSocket.on("connection", (socket) => {
    ...
    socket.on("studentHandRaise", async (data) => {
      console.log("got data ", data);
      try {
        let game = await Game.findOne({
          gameCode: data.gameCode,
        });
        let student = game.roster.filter(
          (student) => student.name == data.student
        );
        if (student[0]) {
          student[0].handRaised = data.hand;
        }
        console.log("got student, ", student);
        game.markModified("roster");
        await game.save();
        console.log(game.roster);
        let returnData = await GetGameData(data.gameCode);
        gameSocket.to(game.teacherSocket).emit("getGameEvent", returnData);
      } catch (err) {
        console.log(err);
      }
    });
